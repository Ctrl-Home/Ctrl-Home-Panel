...
def send_command(brand, product_type, operation, params=None,
                 mqtt_broker_host="mumble.2k2.cc", mqtt_broker_port=11883, topic_prefix="node",
...
def send_status(brand,sensor_name,mqtt_broker_host="mumble.2k2.cc", mqtt_broker_port=11883, topic_prefix="/device"):
    ...
    mqtt_sensor_status = json.dumps(sensor_status)
    print(f"mqtt_sensor_status: {mqtt_sensor_status}")

    if not client:
        return {'success': False, 'message': f"无法连接到 MQTT Broker '{mqtt_broker_host}:{mqtt_broker_port}'。"}
    try:
        # 发送状态消息到指定的主题
        publish.single(topic_path, payload=mqtt_sensor_status, hostname=mqtt_broker_host, port=mqtt_broker_port)
        return {'success': True, 'message': f"成功发送状态消息到主题 '{topic_path}'"}
    except Exception as e:
        error_message = f"发送状态消息到主题 '{topic_path}' 失败: {e}"
        print(error_message)
        return {'success': False, 'message': error_message}
